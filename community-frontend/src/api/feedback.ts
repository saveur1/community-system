...
export const feedbackApi = {
  ...
  // Replies
  getReplies: async (feedbackId: string): Promise<FeedbackReplyEntity[]> => {
    const { data } = await client.get(`/feedback/${feedbackId}/replies`);
    // Controller returns ServiceResponse; normalize to result/data shape if needed
    return (data?.result ?? data?.data ?? data) as FeedbackReplyEntity[];
  },

  addReply: async (feedbackId: string, payload: FeedbackReplyCreateRequest): Promise<FeedbackReplyEntity> => {
    const { data } = await client.post(`/feedback/${feedbackId}/replies`, payload);
    return (data?.result ?? data?.data ?? data) as FeedbackReplyEntity;
  },
};
