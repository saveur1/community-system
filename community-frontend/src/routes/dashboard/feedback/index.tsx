...
import type { FeedbackEntity, FeedbackListParams } from '@/api/feedback';
import { checkPermissions, spacer } from '@/utility/logicFunctions';
import { useAuth } from '@/hooks/useAuth';

const excelDataExported = (feedbacks: FeedbackEntity[]) => {
  return feedbacks?.map((feedback, index) => ({
    id: index + 1,
    mainMessage: feedback?.mainMessage,
    feedbackType: feedback?.feedbackType,
    feedbackMethod: feedback?.feedbackMethod,
    follwUpNeeded: feedback?.followUpNeeded ? "Yes" : "No",
    status: feedback?.status,
    feedbackAbout: feedback.project?.name || feedback?.otherFeedbackOn || "No project selected",
    responder: feedback?.responderName || feedback?.user?.name || "Unknow user",
    responderRole: spacer(feedback?.user?.roles?.[0]?.name || "Unknown"),
    feedbackLink: feedback?.documents?.[0]?.documentUrl || "No uploaded File"
  }))
    ...
    <div className="pb-10">
      ...
      <div className="pt-14">
        <MainToolbar
          ...
          excelColumnWidths={{ id: 6, mainMessage: 40, feedbackType: 16, feedbackMethod: 16, followUpNeeded: 22, status: 14, feedbackAbout: 30, responder: 22, responderRole: 18, feedbackLink: 50, createdAt: 22 }}
          excelFileName='feedbacks'
