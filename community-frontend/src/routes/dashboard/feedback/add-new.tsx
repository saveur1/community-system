        ...
        <div className="flex items-center justify-center mb-6 sm:mb-8">
            {[1, 2].map((step) => (
                <div key={step} className="flex items-center">
                    ...
                    {step < 2 && (
                        <div
                            className={`w-12 sm:w-16 h-1 mx-1 sm:mx-2 ${step < currentStep ? 'bg-green-500' : 'bg-gray-200 dark:bg-gray-700'
                                }`}
                        />
                    )}
                </div>
    ...
    const renderStep1 = () => (
        ...
        >
            <div className="text-center mb-6 sm:mb-8">
                <h2 className="text-xl sm:text-2xl font-bold text-title dark:text-gray-100 mb-2">
            ...
            <div className="space-y-4">
                <label className="block text-sm sm:text-base font-medium text-gray-700 dark:text-gray-300 mb-4">
            ...
            <div>
                <label className="block text-sm sm:text-base font-medium text-gray-700 dark:text-gray-100 mb-4">
                ...
                <div className="space-y-3">
                    {projectsData?.result?.map((project) => (
                        <label
                            key={project.id}
                            className="flex items-center p-3 border border-gray-200 dark:border-gray-600 rounded-lg hover:border-gray-300 dark:hover:border-gray-500 cursor-pointer transition-colors"
                        >
                            <input
                                type="radio"
                                name="projectId"
                                ...
                                className="mr-3 text-primary dark:text-primary-200 focus:ring-primary dark:focus:ring-primary-400 scale-125 sm:scale-150 border-gray-300 dark:border-gray-600 dark:bg-gray-600"
                                aria-label={`Select project ${project.name}`}
                            />
                            <span className="text-gray-900 dark:text-gray-100 text-sm sm:text-base">{project.name}</span>
                        </label>
                    ))}
                    <label className="flex items-center p-3 border border-gray-200 dark:border-gray-600 rounded-lg hover:border-gray-300 dark:hover:border-gray-500 cursor-pointer transition-colors">
                        <input
                            type="radio"
                            name="projectId"
                            value="other"
                        ...
                        />
                        <span className="text-gray-900 dark:text-gray-100 text-sm sm:text-base">Other</span>
                    </label>
                    {feedback.projectId === 'other' && (
                        <div className="ml-6">
                            <input
                                ...
                                className="w-full px-3 sm:px-4 py-2 sm:py-3 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:ring-2 focus:ring-primary dark:focus:ring-primary-400 outline-none focus:border-primary bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400 transition-colors text-sm sm:text-base"
                            />
                        </div>
                    )}
                </div>
    ...
    const renderStep2 = () => (
        <motion.div key="step2" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ duration: 0.3 }} className="space-y-6">
            <div className="text-center mb-8">
                <h2 className="text-2xl font-bold text-title dark:text-gray-100 mb-2">Tell us about your experience</h2>
                <p className="text-gray-600 dark:text-gray-400">Share your feedback about the {selectedProjectName}</p>
            </div>
            <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-4">What type of feedback is this?</label>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {feedbackTypes.map((type) => (
                        <div key={type.value} onClick={() => updateFeedback('feedbackType', type.value as any)} className={`p-4 border-2 rounded-lg cursor-pointer transition-all hover:shadow-md ${feedback.feedbackType === type.value ? 'border-primary bg-blue-50 dark:bg-blue-900/20' : 'border-gray-200 dark:border-gray-600 hover:border-gray-300 dark:hover:border-gray-500'}`}>
                            <div className="flex items-center space-x-3">
                                <span className="text-2xl">{type.icon}</span>
                                <div><h3 className="font-semibold text-gray-900 dark:text-gray-100">{type.label}</h3></div>
                            </div>
                        </div>
                    ))}
                </div>
            ...
            {feedback.feedbackMethod === 'text' && (
                <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Your Message <span className="text-red-500">*</span></label>
                    <textarea value={feedback.mainMessage} onChange={(e) => updateFeedback('mainMessage', e.target.value)} rows={6} className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:ring-2 focus:ring-primary dark:focus:ring-primary-400 outline-none focus:border-primary bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400 transition-colors" placeholder="Please describe your feedback..." />
                </div>
            )}
            ...
            <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
                <label className="flex items-start">
                    <input type="checkbox" checked={feedback.followUpNeeded} onChange={(e) => updateFeedback('followUpNeeded', e.target.checked)} className="mt-1 rounded border-gray-300 dark:border-gray-600 dark:bg-gray-600 text-primary dark:text-primary-200 shadow-sm focus:border-primary focus:ring focus:ring-primary dark:focus:ring-primary-400 focus:ring-opacity-50" />
                    <div className="ml-3">
                        <span className="text-sm font-medium text-gray-900 dark:text-gray-100">Request Follow-up</span>
                        <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">Check this if you would like someone to follow up on this feedback</p>
                    </div>
                </label>
            </div>
        </motion.div>
    ...
    return (
        <div className="w-full">
            ...
            <div className="max-w-4xl mx-auto sm:px-4 max-sm:px-2 lg:px-8 pt-20 pb-8 sm:pb-12 w-full">
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg drop-shadow-2xl border border-gray-200 dark:border-gray-700">
                    <div className="p-6 sm:p-8 max-sm:px-3">
                        ...
                        <div className="flex justify-between items-center mt-6 sm:mt-8 pt-6 border-t border-gray-200 dark:border-gray-700">
                            <button
                                type="button"
                                onClick={currentStep === 1 ? handleCancel : prevStep}
                                className="flex items-center px-4 sm:px-6 py-2 sm:py-3 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 font-medium transition-colors text-sm sm:text-base"
                            >
                            ...
                            {currentStep < totalSteps ? (
                                <button
                                    ...
                                    className="flex items-center px-4 sm:px-6 py-2 sm:py-3 bg-primary dark:bg-primary/90 text-white rounded-lg hover:bg-primary-dark dark:hover:bg-primary font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-sm sm:text-base"
                                >
                                ...
                                <button
                                    ...
                                    className="flex items-center px-4 sm:px-6 py-2 sm:py-3 bg-primary dark:bg-primary/90 text-white rounded-lg hover:bg-primary-dark dark:hover:bg-primary font-medium transition-colors disabled:opacity-50 text-sm sm:text-base"
                                >
