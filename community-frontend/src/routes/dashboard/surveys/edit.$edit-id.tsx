...
function EditSurveyComponent(): JSX.Element {
  ...
  // Confirmation modal for deleting a section
  ...
  // Renumber helper: ensure questionNumber matches visible order across sections
  const renumberQuestions = (questions: Question[], currentSections: Section[]): Question[] => {
    const next = [...questions];
    let num = 1;
    for (const section of currentSections) {
      const inSection = next.filter(q => q.sectionId === section.id);
      inSection.forEach(q => {
  ...
  const deleteSection = (sectionId: string): void => {
    ...
    if (currentSectionId === sectionId) {
      setCurrentSectionId(updatedSections[0]?.id)
    }
  ...
  // Keep question numbers in sync when sections change/current section changes
  useEffect(() => {
    setSurvey(prev => ({ ...prev, questions: renumberQuestions(prev.questions, sections) }));
  ...
  const updateQuestion = (questionId: number, field: string, value: any): void => {
    setSurvey(prev => {
      const updated = prev.questions.map(q => (q.id === questionId ? ({ ...(q as any), [field]: value } as any) : q));
  ...
  const duplicateQuestion = (questionId: number): void => {
    ...
    setSurvey(prev => {
      const appended = [...prev.questions, duplicated];
      const renumbered = renumberQuestions(appended, sections);
      return { ...prev, questions: renumbered };
    });
  };
