...
const ReportFormDetail = () => {
  ...
  const [activeTab, setActiveTab] = useState<'questions' | 'responses' | 'collectors'>('questions');
  ...
  // const { data: surveyResponses, isLoading: isLoadingResponses } = useSurveyResponses(reportId, page, pageSize, true);
  ...
  // Extract unique data collectors from survey responses
  const uniqueCollectors = useMemo(() => {
    const collectorsMap = new Map();
    
    for (const response of surveyResponses as any[]) {
      if (response.user && response.user.id) {
        const user = response.user;
        if (!collectorsMap.has(user.id)) {
          collectorsMap.set(user.id, {
            id: user.id,
          ...
          const existing = collectorsMap.get(user.id);
  ...
  // Create collector options for the filter
  const collectorOptions = useMemo(() => {
    const options = [{ label: 'All Collectors', value: 'all' }];
    uniqueCollectors.forEach(collector => {
      options.push({
  ...
  const filtered = useMemo(() => {
    ...
    let filtered = fetchedResponses.filter((r) =>
      r.respondentName.toLowerCase().includes(term) ||
      String(r.id).includes(term) ||
      (r.respondentRole ?? '').toLowerCase().includes(term)
    );
    
    // Filter by selected collector if one is selected
    if (selectedCollector && selectedCollector !== 'all') {
      filtered = filtered.filter((r) => r.respondentName === selectedCollector);
    }
    
    return filtered;
  }, [fetchedResponses, search, selectedCollector]);
                  ...
                  <div key={collector.id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                    <div className="flex items-start space-x-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {collector.roles && collector.roles.length > 0 && (
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                ...
                <div className="flex items-center gap-3">
                  <SelectDropdown
                    ...
                    value={selectedCollector || 'all'}
                    onChange={(value) => { setSelectedCollector(value === 'all' ? '' : value); setPage(1); }}
