...
const SurveyAnswerForm: React.FC<SurveyAnswerFormProps> = ({ onComplete, survey }) => {
  ...
  const orderedSections = (survey.sections ?? []).slice().sort((a, b) => (b.order ?? 0) - (a.order ?? 0) );
  ...
  // Compute global question numbering start for this section (sum previous sections' counts)
  const startNumberForCurrentSection = steps.slice(0, currentStep).reduce((acc, s) => acc + s.questions.length, 0);
