...
import { useGetFeedbackReplies } from '@/hooks/useFeedback';
...
interface FeedbackDetailsDrawerProps {
    ...
    onReply?: (feedback: FeedbackEntity) => boolean;
...
export const FeedbackDetailsDrawer: React.FC<FeedbackDetailsDrawerProps> = ({
    ...
    const { data: replies = [], isLoading: isRepliesLoading } = useGetFeedbackReplies(feedback?.id || '');
    console.log("Replies", replies);

    const initialFeedbackBubble = useMemo(() => {
        if (!feedback?.mainMessage) return null;
