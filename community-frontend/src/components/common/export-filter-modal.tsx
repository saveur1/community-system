    ...
    <Modal isOpen={isOpen} onClose={() => { reset(); onClose(); }} title={title} size="xl">
      <ModalBody>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <div className="mb-2 text-sm font-medium text-gray-700">Start Date & Time</div>
            <DateTimePicker
              ...
              trigger={
                <input
                  ...
                  className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm bg-white cursor-pointer hover:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors"
          ...
          <div>
            <div className="mb-2 text-sm font-medium text-gray-700">End Date & Time</div>
            <DateTimePicker
        ...
        {columns.length > 0 && (
          <div className="mt-6">
            <div className="mb-2 text-sm font-medium text-gray-700">Columns to export</div>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
              {columns.map((col) => {
                const id = `col-${col.key}`;
                const checked = selectedColumns.includes(col.key);
                return (
                  <label key={col.key} htmlFor={id} className="inline-flex items-center gap-2 text-sm text-gray-700">
                    <input
                      id={id}
                      type="checkbox"
