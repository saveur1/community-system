    ...
    <div className={`flex flex-col sm:flex-row sm:justify-between w-full bg-white px-4 max-sm:px-2 py-2 my-6 border border-gray-300 rounded-md items-center mb-6 ${className}`}>
        ...
        <div className="flex-1 w-full sm:w-auto sm:mt-0 ml-3">
          <div className="relative w-full">
            <input
              ...
              className={`w-full max-w-lg border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary ${isSearchOpen ? 'visible' : 'invisible sm:visible'}`}
            />
            <button
      ...
      <div className="flex items-center justify-end gap-2 w-1/3 max-sm:w-full max-sm:justify-center max-sm:gap-1 mt-2 sm:mt-0 sm:ml-3">
        ...
        {showCreate && (
          <Link
            to={createBtn.to}
            className={`bg-primary text-white px-4 max-sm:px-2 py-2 max-sm:py-1.5 rounded-lg hover:bg-primary-dark transition-colors font-medium shadow-sm max-sm:text-xs flex items-center gap-1.5 ${createBtn?.className ?? ''}`}
          >
