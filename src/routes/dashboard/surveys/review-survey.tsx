...
function RouteComponent() {
  ...
  const answerMap = myAnswers.reduce<Record<string, { answerText: string | null; answerOptions: string[] | null }>>((acc: any, answer: any) => {
    if (answer.questionId) {
      acc[answer.questionId] = { answerText: answer.answerText ?? null, answerOptions: answer.answerOptions ?? null };
    }
    return acc;
  }, {});
                    ...
                    <div className="mt-4">
                        ...
                        <div className="space-y-2">
                          {question.options.map((option) => (
                            <div key={option} className="flex items-center">
                              <input type="radio" checked={answerValue === option} readOnly className="h-4 w-4 text-primary/30 border-gray-300 opacity-60" />
                      ...
                      {question.type === 'multiple_choice' && question.options && (
                        <div className="space-y-2">
                          {question.options.map((option) => (
                            <div key={option} className="flex items-center">
                              <input type="checkbox" checked={answer?.answerOptions?.includes(option) || false} readOnly className="h-4 w-4 text-primary/30 border-gray-300 rounded opacity-60" />
                              <label className="ml-3 block text-sm font-medium text-gray-700">{option}</label>
                            </div>
                          ))}
                        </div>
                      ...
                      {(question.type === 'text_input' || question.type === 'textarea') && (
                        <div className="mt-1">
                          <div className="mt-1 block w-full rounded-md bg-gray-50 border-gray-200 border p-2 text-gray-700">{answerValue || 'No answer provided'}</div>
                        </div>
                      )}
                          ...
                          {(answer?.answerOptions?.length ?? 0) > 0 ? (
                            <div className="flex flex-wrap gap-2">
                              {answer?.answerOptions?.map((name: string) => (
                                <span key={name} className="inline-flex items-center px-2 py-1 text-xs bg-gray-100 text-gray-700 rounded">{name}</span>
                              ))}
                            </div>
                          ) : (
                      ...
                      {question.type === 'rating' && (
                        <div className="mt-1 text-sm text-gray-700">
                          Rating: <span className="font-medium">{answer?.answerText ?? '—'}</span>{question.ratingLabel ? ` ${question.ratingLabel}` : ''}
                        </div>
                      )}
                      {question.type === 'linear_scale' && (
                        <div className="mt-1">
                          <div className="flex items-center gap-3">
                            ...
                            <input type="range" min={question.minValue ?? 1} max={question.maxValue ?? 5} step={1} value={Number(answer?.answerText ?? question.minValue ?? 1)} readOnly className="w-full" />
                            <span className="text-xs text-gray-500">{question.maxLabel ?? question.maxValue}</span>
                          </div>
                          <div className="text-sm text-gray-700 mt-1">Selected: <span className="font-medium">{answer?.answerText ?? '—'}</span></div>
                        </div>
                      )}
                    </div>
                  </div>
