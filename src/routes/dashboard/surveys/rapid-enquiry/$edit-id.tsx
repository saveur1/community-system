...
function RouteComponent() {
  ...
  useEffect(() => {
    try {
      ...
      if (!raw) { toast.error('Not found'); navigate({ to: '/dashboard/surveys/rapid-enquiry' }); return; }
      const arr = JSON.parse(raw) as any[];
      const found = arr.find((a) => String(a.id) === id);
      if (!found) { toast.error('Not found'); navigate({ to: '/dashboard/surveys/rapid-enquiry' }); return; }
      setItem(found);
      ...
      <div className="pt-20 max-w-3xl mx-auto">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div className="flex justify-between items-center mt-6">
            <button onClick={() => navigate({ to: '/dashboard/surveys/rapid-enquiry' })} className="px-4 py-2 rounded-md border flex items-center gap-2">
              <FiChevronLeft /> Back
            </button>
