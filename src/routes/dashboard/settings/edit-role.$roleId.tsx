...
import { FiChevronLeft, FiChevronRight, FiUsers, FiShield } from 'react-icons/fi';
...
const EditRolePage = () => {
  ...
  if (roleLoading) {
    return (
      <div>
        <Breadcrumb 
          title="Edit Role" 
          items={['Dashboard', 'Settings', 'Edit Role']} 
    ...
    return (
      <div>
        ...
        <div className="pt-16 max-w-4xl mx-auto">
          <div className="bg-white rounded-lg shadow-lg drop-shadow-xl p-8">
            <div className="text-center py-12">
              <div className="text-red-600">Error loading role: {roleError.message}</div>
              <button 
  ...
  return (
    <div>
      <Breadcrumb 
        title={`Edit ${roleName}`} 
        items={['Dashboard', 'Settings', `Edit ${roleName}`]} 
        className="absolute top-0 left-0 w-full"
      />
